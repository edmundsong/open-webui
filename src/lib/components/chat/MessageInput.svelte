										...
										{#if !history?.currentId || history.messages[history.currentId]?.done == true}
											{#key [attestationValid, JSON.stringify(attestationInfo)]}
												<Tooltip content={`Intel TDX attestation information:<br>` +
													(attestationInfo && attestationInfo.quote_parse ?
													Object.entries(attestationInfo.quote_parse).map(([key, value]) => `${key}:<br>${value}`).join('<br>')
